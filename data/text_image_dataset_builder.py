...
SOURCES_ROOT = (
    Path(os.environ.get("OPTIZIP_SOURCES", str(_DATA_DIR / "sources")))
    .expanduser()
    .resolve()
)
...
class Config:
    ...
    datasets: List[DatasetEntry] = field(
        default_factory=lambda: [
            ...
            DatasetEntry(
                name="cpp_firefox",
                source_root=str(SOURCES_ROOT / "firefox"),
                include_dirs=[
                    "mfbt",
                    "xpcom",
                    "dom",
            ...
            DatasetEntry(
                name="python_cpython",
                source_root=str(SOURCES_ROOT / "cpython"),
                include_dirs=["Lib", "Tools"],
                extensions=[".py", ".pyi"],
            ...
            DatasetEntry(
                name="rust_rustc",
                source_root=str(SOURCES_ROOT / "rust"),
                include_dirs=["library", "compiler"],
                extensions=[".rs"],
            ...
            DatasetEntry(
                name="c_linux",
                source_root=str(SOURCES_ROOT / "linux"),
                include_dirs=[
                    "include/linux",
                    "include/asm-generic",
                    "kernel/locking",
            ...
            DatasetEntry(
                name="asm_nasm",
                source_root=str(SOURCES_ROOT / "nasm"),
                include_dirs=["macros", "test", "travis", "misc"],
            ...
            DatasetEntry(
                name="java_spring",
                source_root=str(SOURCES_ROOT / "spring"),
                include_dirs=[
            ...
            DatasetEntry(
                name="csharp_roslyn",
                source_root=str(SOURCES_ROOT / "roslyn"),
                include_dirs=[
    ...
    font_sizes: List[int] = field(
        default_factory=lambda: [10, 11, 12, 13, 14, 15, 16, 18, 20]
    )
...
class MultiLangCodeParser:
    ...
    @staticmethod
    def find_smart_split(
        lines: List[str], target_lines: int, comment_char: str = "//"
    ) -> int:
    ...
    def extract_snippets(
        ...
        for start in start_candidates:
            ...
            end = MultiLangCodeParser.find_smart_split(lines, target_end, comment_char)
            ...
            if min_lines <= len(snippet_lines) <= max_lines:
                ...
                code_lines = [
                    ...
                    if l.strip()
                    and not l.strip().startswith((comment_char, "#", ";", "/*", "*"))
                ]
...
def generate_dataset(config: Config):
    ...
    config_dict = {k: v for k, v in config.__dict__.items() if k != "datasets"}
    ...
    for ds in config.datasets:
        ...
        if n_allocated > n_available:
            ...
            print(
                f"   {ds.name:20s}: {n_allocated:>6,} sélectionnés (suréchantillonnage, {n_available} disponibles)"
            )
        else:
            sampled = random.sample(snippets, n_allocated)
            print(
                f"   {ds.name:20s}: {n_allocated:>6,} sélectionnés (sur {n_available} disponibles)"
            )
    ...
    with open(mapping_file, "w") as f:
        f.write(
            "index,dataset,style,font_size,dpi,line_numbers,char_count,line_count\n"
        )

        for i, (ds_name, snippet, lexer) in enumerate(
            tqdm(selected_snippets, desc="   Rendu")
        ):
            ...
            snippet_lines = snippet.split("\n")
            f.write(
                f"{generated},{ds_name},{style},{font_size},{dpi},{line_numbers},"
    ...
    print(
        f"   Taille moyenne img:  {total_img_size / generated / 1024:.1f} KB"
        if generated
        else ""
    )
    print(
        f"   Taille moyenne txt:  {total_txt_size / generated / 1024:.1f} KB"
        if generated
        else ""
    )
